...
router.get("/stream", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const unsubscribe = colRef.orderBy("createdAt", "asc").onSnapshot(async (snap) => {
      if (snap.empty) {
        try {
      ...
      const items = snap.docs.map((d) => {
        const data = d.data() as { name: string; createdAt?: Timestamp; orderIndex?: number };
        return { id: d.id, name: data.name, createdAt: (data.createdAt as Timestamp) || Timestamp.now(), _orderIndex: typeof data.orderIndex === "number" ? data.orderIndex : null } as any;
      });
...
router.post("/", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!exists.empty) {
      res.status(409).json({ status: "error", message: "A watchlist with this name already exists" });
      return;
    }
    const doc = await colRef.add({ name: trimmed, createdAt: FieldValue.serverTimestamp() });
...
router.patch("/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await colRef.doc(id).set({ name: trimmed, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
...
router.post("/reorder", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    order.forEach((wid, idx) => {
      ...
      batch.set(ref, { orderIndex: idx, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
    });
    await batch.commit();
    res.json({ status: "success" });
  } catch (error: any) {

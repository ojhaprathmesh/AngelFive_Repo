...
export function DashboardNavbar({ user }: DashboardNavbarProps) {
    ...
    const MarketIndicator = ({
        ...
        if (marketInfo.isLoading) {
            return (
                <div
                    className={`flex flex-col space-y-1 ${isCompact ? "items-center" : ""
                        }`}
                >
        ...
        if (marketInfo.error) {
            return (
                <div
                    className={`flex flex-col ${isCompact ? "items-center text-center" : ""
                        }`}
                >
        ...
        return (
            ...
            >
                <div className="flex items-center space-x-1">
                    <span
                        className={`font-medium text-gray-900 dark:text-gray-100 ${isCompact ? "text-xs" : "text-xs"
                            }`}
                    >
                        {data.symbol}
                    </span>
                    <span
                        className={`font-semibold text-gray-900 dark:text-gray-100 ${isCompact ? "text-xs" : "text-xs"
                            }`}
                    >
                    ...
                    {!isCompact && (
                        <div
                            className={`h-2 w-2 rounded-full ${isDataFresh ? "bg-green-500" : "bg-yellow-500"
                                }`}
    ...
    return (
        <nav
            className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white/95 backdrop-blur supports-backdrop-filter:bg-white/60 dark:border-gray-800 dark:bg-gray-950/95 dark:supports-backdrop-filter:bg-gray-950/60 safe-top">

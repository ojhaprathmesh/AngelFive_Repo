...
export function PortfolioOptimization() {
    ...
    const renderEfficientFrontier = (frontier: any[], optimal: any) => {
        ...
        const allPoints = optimal
            ? [...sortedFrontierClean, optimal]
            : sortedFrontierClean;
                        ...
                        <Button
                            onClick={async () => {
                                if (selectedSymbols.length < 2) {
                                    alert(
                                        "Please select at least 2 stocks for portfolio optimization",
                                    );
                                    return;
                                }
                                setLoadingMPT(true);
                        ...
                        <Button
                            onClick={async () => {
                                ...
                                try {
                                    ...
                                    alert(
                                        `Black-Litterman Error: ${e.message || "Network error"}`,
                                    );
                                } finally {
                                    setLoadingBL(false);
                                }
                                            ...
                                            <div className="mt-4">
                                                ...
                                                <div
                                                    ...
                                                    {blResult.symbols?.map((sym: string, idx: number) => {
                                                        const weight =
                                                            blResult.optimal_weights[idx] * 100 || 0;
                                                        if (weight < 0.1) return null; // Skip very small weights
                                                        return (
                                                            <div
                                                                key={sym}

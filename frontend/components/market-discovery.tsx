...
interface DiscoveryResponse {
  ...
  topLosers: QuoteLike[];
  pocketFriendly: { under50: QuoteLike[]; under100: QuoteLike[]; under200: QuoteLike[] };
}
...
export default function MarketDiscovery() {
  ...
  const [technicalScreeners, setTechnicalScreeners] = useState<TechnicalScreenerItem[]>([]);
  ...
  useEffect(() => {
    const loadMovers = async () => {
      try {
        const resp = await fetch(`/api/market/gainers-losers`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ datatype: 'PercPriceGainers', expirytype: 'NEAR' }) });
        if (resp.ok) {
          const jl: { source: string; items?: Array<{ tradingSymbol: string; percentChange?: number }> } = await resp.json();
          if (jl.source === 'smartapi' && Array.isArray(jl.items)) {
            const mapItem = (x: { tradingSymbol: string; percentChange?: number }): MarketData => ({ symbol: x.tradingSymbol, price: 0, change: 0, changePercent: Number(x.percentChange || 0), lastUpdated: new Date().toISOString() });
            const arr = jl.items.map(mapItem);
            setGainers(arr.slice(0, 8));
            // losers call:
            const resp2 = await fetch(`/api/market/gainers-losers`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ datatype: 'PercPriceLosers', expirytype: 'NEAR' }) });
            if (resp2.ok) {
              const jl2: { source: string; items?: Array<{ tradingSymbol: string; percentChange?: number }> } = await resp2.json();
    ...
    const loadPerf = async () => {
      try {
        ...
        if (resp.ok) {
          ...
          console.log(`[Frontend] Loaded ${mapped.length} performers for ${tf}`);
  ...
  const header = (title: string, section?: string) => (
    <div className="flex items-center justify-between mb-3">
      ...
      {section && (
        <a href={`/dashboard/market/view-all?section=${encodeURIComponent(section)}`} className="text-xs text-blue-600">VIEW ALL</a>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-6 w-full">
      <div className="border rounded-lg p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          {(!loading && mostBought.length === 0) && (
            <div className="text-xs text-gray-500">No data available right now.</div>
          )}
            ...
            <div className="divide-y">
              ...
              {(loading ? [] : gainers).slice(0,5).map((g, i) => (
                <div key={i} className="flex items-center justify-between py-2 text-sm">
                  ...
                  <div className="w-24 text-right text-green-700 flex items-center gap-1 justify-end"><TrendingUp className="h-3 w-3" /> {g.change.toFixed(2)}</div>
                  <div className="w-24 text-right"><PriceBadge value={g.changePercent} /></div>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="divide-y">
              ...
              {(loading ? [] : losers).slice(0,5).map((g, i) => (
                <div key={i} className="flex items-center justify-between py-2 text-sm">
                  ...
                  <div className="w-36 text-right">₹{g.price.toFixed(2)}</div>
                  <div className="w-24 text-right text-red-700 flex items-center gap-1 justify-end"><TrendingDown className="h-3 w-3" /> {g.change.toFixed(2)}</div>
                  <div className="w-24 text-right"><PriceBadge value={g.changePercent} /></div>
                </div>
              ))}
            </div>
      ...
      <div className="border rounded-lg p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <Tabs value={tf} onValueChange={(v: string) => setTf(v as "1W" | "1M" | "1Y" | "5Y")}>
            <TabsList>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {loadingPerformers && (
            <div className="col-span-full text-xs text-gray-500 text-center py-4">Loading performers for {tf}...</div>
          )}
          {!loadingPerformers && performers.length === 0 && (
            <div className="col-span-full text-xs text-gray-500 text-center py-4">No performers data available for {tf}</div>
          )}
          {!loadingPerformers && performers.map((p, i) => (
            <div key={i} className="border rounded-lg p-3">
              <div className="text-sm font-semibold">{p.symbol}</div>
              <div className="text-xs text-gray-500">₹{p.ltp.toFixed(2)}</div>
              <div className="mt-2"><PriceBadge value={p.changePct} /></div>
            </div>
          ))}
        </div>
      </div>
          ...
          {technicalScreeners.map((s, idx) => (
            <div key={idx} className="border rounded-lg p-3">
              ...
              <div className="text-xs text-gray-500">
                {s.signal === 'BULLISH' && <span className="text-green-600 font-medium">🟢 BULLISH (EMA)</span>}
                {s.signal === 'BEARISH' && <span className="text-red-600 font-medium">🔴 BEARISH (EMA)</span>}
                {s.signal === 'NEUTRAL' && <span className="text-gray-600 font-medium">⚪ NEUTRAL (EMA)</span>}
              </div>
              <div className="flex items-center gap-2">
                <span className="text-sm">₹{s.price.toFixed(2)}</span>
                <PriceBadge value={s.changePercent} />
      ...
      <div className="border rounded-lg p-4">
        ...
        <div className="flex items-center gap-2 mb-3">
          <button className={`px-2 py-1 text-xs rounded ${pfTab==='50'?'bg-blue-600 text-white':'bg-gray-100'}`} onClick={()=>setPfTab('50')}>≤50</button>
          <button className={`px-2 py-1 text-xs rounded ${pfTab==='100'?'bg-blue-600 text-white':'bg-gray-100'}`} onClick={()=>setPfTab('100')}>≤100</button>
          <button className={`px-2 py-1 text-xs rounded ${pfTab==='200'?'bg-blue-600 text-white':'bg-gray-100'}`} onClick={()=>setPfTab('200')}>≤200</button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          {(pfTab==='50'?pf50:pfTab==='100'?pf100:pf200).slice(0,5).map((s, idx) => (
            <div key={idx} className="border rounded-lg p-3">
              <div className="text-sm font-semibold">{s.symbol}</div>
              <div className="flex items-center gap-2">
                <span className="text-sm">₹{s.price.toFixed(2)}</span>
      ...
      <div className="border rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-medium text-gray-900 dark:text-white">Join our Community</h3>
        </div>

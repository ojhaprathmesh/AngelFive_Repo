...
import { X, Link as LinkIcon, ChevronRight, ChevronLeft } from "lucide-react";
...
export function OptionQuickList({ isOpen, onClose }: OptionQuickListProps) {
  const [selectedTab, setSelectedTab] = useState<"NIFTY" | "BANKNIFTY" | "FINNIFTY">("NIFTY");
  ...
  // Mock expiry dates
  const expiries = ["28 Nov 25", "05 Dec 25", "12 Dec 25", "19 Dec 25", "26 Dec 25"];
  ...
  useEffect(() => {
    ...
    const fetchOptionChain = async () => {
      ...
      try {
        ...
        const marketData = await marketDataService.getMarketDataWithStatus(symbolMap[selectedTab]);
        const ltp = marketData.data?.price || (selectedTab === "NIFTY" ? 24000 : selectedTab === "BANKNIFTY" ? 52000 : 24000);
        setUnderlyingPrice(ltp);

        // Generate strikes
        const step = selectedTab === "NIFTY" ? 50 : selectedTab === "BANKNIFTY" ? 100 : 50;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center p-0 sm:p-4 pointer-events-none">
      <div className="bg-white dark:bg-gray-900 w-full max-w-4xl h-[80vh] sm:h-[600px] rounded-t-xl sm:rounded-xl shadow-2xl border border-gray-200 dark:border-gray-800 flex flex-col pointer-events-auto">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800">
          <h2 className="text-base font-semibold text-gray-900 dark:text-white">Option Quicklist</h2>
          ...
          <div className="flex items-center gap-6">
            {(["NIFTY", "BANKNIFTY", "FINNIFTY"] as const).map((tab) => (
              <button
                ...
                className={`text-sm font-medium transition-colors relative pb-2 -mb-[13px] ${
                  selectedTab === tab
        ...
        {/* Expiry Scroll */}
        <div className="px-4 py-2 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-800 flex items-center gap-2 overflow-x-auto no-scrollbar">
          {expiries.map((date) => (
            <button
              key={date}
              onClick={() => setExpiryDate(date)}
              className={`px-3 py-1.5 rounded text-xs font-medium whitespace-nowrap border transition-colors ${
                expiryDate === date
          ...
          {loading && optionData.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                {optionData.filter(x => x.type === "CE").map((opt) => (
                  <OptionCard key={`${opt.strikePrice}-CE`} data={opt} />
                ))}
              </div>
              {/* PE Column */}
              <div className="space-y-2">
                {optionData.filter(x => x.type === "PE").map((opt) => (
...
function OptionCard({ data }: { data: OptionData }) {
  const isITM = data.type === "CE" ? data.ltp > data.strikePrice : data.ltp < data.strikePrice;
  const isPositive = data.change >= 0;
  
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-3 hover:shadow-sm transition-shadow cursor-pointer group">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <Badge 
            variant="secondary" 
            className={`text-[10px] h-5 px-1.5 rounded ${
              isITM 
      ...
      <div className="flex items-center justify-between text-xs">
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button className="bg-green-600 hover:bg-green-700 text-white px-3 py-0.5 rounded text-[10px] font-medium">B</button>
          <button className="bg-red-600 hover:bg-red-700 text-white px-3 py-0.5 rounded text-[10px] font-medium">S</button>
        </div>
